...
const steps = [
  ...
  ["使用空闲提醒", "需要充电时开启，找到空位后自动停止", ["在看板中找到目标充电桩，点击“有空闲时提醒我”。", "选择等待 1、2、4 小时或持续到今晚断电前，然后点击“开始提醒”。", "系统会先查看一次当前状态；如果已经有空闲口，会直接告诉你端口号，不再创建后台任务。", "没有空闲口时，提醒会在所选时间内运行；发现空闲、等待到期或手动取消后都会自动停止。", "点击看板顶部的“空闲提醒”，可以查看剩余时间和下次检查，也可以延长或取消。", "如有每周固定需求，可在“固定时段提醒（高级）”中设置星期和时间。", "如需浏览器弹窗提醒，请在通知中心点击“允许通知”，并保持网页打开。"]],
] as const
...
export function UsageGuideDialog() {
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== "user" || user.usageGuideAckAt || promptedRef.current === user.id) return
