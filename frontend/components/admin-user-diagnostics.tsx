...
const actionableCodes = new Set([
  "pile_identifier_required", "pile_id_invalid", "pile_number_invalid", "pile_fields_invalid", "pile_port_count_invalid",
...
export function AdminUserDiagnostics({ diagnostics }: { diagnostics: RecoveryDiagnostic[] }) {
  ...
  return <div className="border-t pt-3">
    <Button type="button" variant="ghost" size="sm" onClick={() => setExpanded((value) => !value)}>
      {expanded ? `收起诊断（${failures.length}）` : `查看诊断（${failures.length}）`}
    </Button>
    {expanded ? <ul className="mt-2 space-y-2">
      {failures.map((item) => <li key={`${item.code}-${item.at}`} className="rounded-md bg-muted/40 p-2 text-xs">
        <p className="font-medium">{item.message}</p>
        <p className="mt-1 text-muted-foreground">{operationLabel[item.operation]} · {new Date(item.at).toLocaleString("zh-CN")}{item.deviceSuffix ? ` · 设备尾号 ${item.deviceSuffix}` : ""}{item.statusCode ? ` · 状态码 ${item.statusCode}` : ""}</p>
      </li>)}
    </ul> : null}
  </div>
}
